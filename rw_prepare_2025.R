...
nrow(pairs_universe) # 395.898.227; 229.238.862
...
for (i in seq_along(QUARTERS)) {
  ...
  chunks = split(pairsy, cut(1:nrow(pairsy), breaks = 70))
...
head(test, 20)
tail(test, 20)
pairs_features[, roll_lsr_4 := frollmean(lsr_bucket, 4), by = .(stock1, stock2)]
pairs_features[, roll_lsr_8 := frollmean(lsr_bucket, 8), by = .(stock1, stock2)]
pairs_features[, roll_ed_4 := frollmean(ed_bucket, 4), by = .(stock1, stock2)]
pairs_features[, roll_ed_8 := frollmean(ed_bucket, 8), by = .(stock1, stock2)]
pairs_features[, combo_score_4 := (roll_lsr_4 + roll_ed_4) / 2]
pairs_features[, combo_score_8 := (roll_lsr_8 + roll_ed_8) / 2]
na.omit(pairs_features[stock1 == "AAPL"])
# TODO: TRY to find faster weghted mean than above

# Check number of NA's
pairs_features[, sort(unique((q)))]
pairs_features[q > 2023.5][, sum(is.na(combo_score_4)) / nrow(pairs_features) * 100]

# Remove missing values
pairs_combo = na.omit(pairs_features)
