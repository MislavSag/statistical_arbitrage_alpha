...
# Remove pairs with same first three leters in ticker (probabbly same firm)
unique(pairs[name1 == name2, .(name1, name2)])
pairs = pairs[name1 != name2]

# Descriptive
pairs[, .N, by = q][order(q)] |>
  ggplot(aes(x = q, y = N)) +
...
# Parameters
np  = 0.005 # Choose n perccent of best pairs by combo score
var = "combo_score_8"

# Select n best by var
setorderv(pairs, cols = c("q", var), order = -1)
head(pairs[q == 2025])
dt = pairs[, head(.SD, as.integer(np * length(stock1))), by = q]
...
print(paste("Removed", length(removed), "symbols with less than 50 occurrences", "and", length(keep_symbols), "symbols kept"))
...
dt = dt[, .(q, stock1, stock2)]
dt[, date := zoo::as.Date.yearqtr(q)]
...
# rolling mean for mean zscore
# setorder(mean_zscore, stock, date)
# mean_zscore[, mean_zscore_5 := frollmean(mean_zscore, 5L, na.rm = TRUE)]
# mean_zscore[, mean_zscore_10 := frollmean(mean_zscore, 10L, na.rm = TRUE)]
# setorder(mean_zscore, date, -mean_zscore)
